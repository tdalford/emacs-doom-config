;; my-doom-monokai-classic-theme.el --- inspired by Textmate's Monokai -*- lexical-binding: t; no-byte-compile: t; -*-
(require 'doom-themes)
...
;;
(def-doom-theme my-doom-monokai-classic
  ...
  ((bg         '("#272822" nil       nil          ))
   ...
   (comments       (if doom-monokai-classic-brighter-comments cyan base5))
   (doc-comments   (if doom-monokai-classic-brighter-comments (doom-lighten yellow 0.1) (doom-lighten base5 0.25)))
   (constants      violet)
   (functions      green)
   (keywords       magenta)
   (methods        green)
   (operators      magenta)
   (type           orange) ;; change from blue to orange
   (strings        yellow)
   (variables      orange) ;; change to orange from fg
   (numbers        violet)
   (region         base4) ;; I like base4 actually
   (error          red)
   (warning        yellow)
   (success        green)
   (vc-modified    cyan)
  ...
  ((cursor :background magenta)
   ...
   ;; make swiper better- current match is too dark right now
   ;; can also customize swiper-line-face!
   (ivy-current-match :background grey) ;;was base3
   (ivy-minibuffer-match-face-1 :background base1 :foreground base4)
   ;; change the swiper match to just highlight in violet
   (ivy-minibuffer-match-face-2
    :inherit 'ivy-minibuffer-match-highlight)
   ;; no background for swiper line
   (swiper-line-face :background nil)
   ;;;; markdown-mode
   ...
   ;;;; rainbow-delimiters
   ;; change to magenta first-- just reverse list!
   ...
   ;; make the line highlight lighter instead of darker
   (hl-line :background "#3C3D37" :extend t)

   ;; change mu4e to not highlight weirdly
...
;;; my-doom-monokai-classic-theme.el ends here
